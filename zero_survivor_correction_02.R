...
input <- fread(paste0(input_dir, '/allsibhistories_formatted.csv'))
...
for (survey in unique(input$svy)) {
  ...
  # some surveys don't have v021, guess we'll have to comment out
  ...
  # only keep siblings in sibships with 1 or 2 sibs - females only for now, not sure how to do for males
  ...
  #Decision Point: change the age range if diff than 15-49, line125
  ...
  # merge weights into sister
  # sibs <- merge(sibs, weights, by='cohort', all.x=T)
  # sibs[, c('dead','birth','q') := NULL]
  ...
  # like in STATA, all missing sibs from 1-sibling sibships for now
...
# v005 is a population weight, why don't some sets have it? Not sure
...
# fill in missing ageresps
# all_sibs[is.na(ageresp), ageresp := ifelse(is.na(yod), yr_interview-yob, yod-yob)]
...
# Write output
fwrite(all_sibs, paste0(output_dir, '/sibhistories_with_missing_sibs.csv'))
